...
<%-- 	<input type="text" name="tiper_img" id="tiper_img" 	value="${lessonDetail.tiper_img}"> --%>
		...
		<div id="contbox0">
			...
			<c:if test="${lessonDetail.lesson_cate eq '공예' }"><img src="${pageContext.request.contextPath}/front/Craftdefault.jpg" alt="공예카테고리"></c:if>
			<c:if test="${lessonDetail.lesson_cate eq '사무' }"><img src="${pageContext.request.contextPath}/front/Officedefault.jpg" alt="사무카테고리"></c:if>
			<c:if test="${lessonDetail.lesson_cate eq '라이프스타일' }"><img src="${pageContext.request.contextPath}/front/Lifestyledefault.jpg" alt="라이프스타일카테고리"></c:if>
		</div>
...
//Number로 변환하지 않으면 아래에서 +를 할 경우 스트링 결합이 되어버림.. 
 ...
 if( typeof chartHtml == "undefined" || chartHtml == ''){ chartHtml += "<div id='cont3_noreview'>리뷰가 없어요</div>"}
	console.log("chartHtml이 body에 입력되기 전에 : " + chartHtml);
$("#data_table_body").html(chartHtml);
}
